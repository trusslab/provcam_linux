...
// replay related
#include "xilinx-vpss-csc-presets.h"
#include <linux/secure-cam-rep.h>
u8 xcsc_replay_status = 0;	// 0: init; 1: start; 2: stop; other: invalid

void check_and_switch_to_next_presets_4_xcsc(void)
{
	u8 previous_replay_status = xcsc_replay_status - 1;
	while (previous_replay_status != xcsc_replay_status)
	{
		previous_replay_status = xcsc_replay_status;
		switch(xcsc_replay_status)
		{
			case 0:
				if (check_and_switch_current_replay_preset(xcsc_init_preset_num_of_commands, xcsc_init_preset_cmd_type, 
						xcsc_init_preset_size, xcsc_init_preset_addr, xcsc_init_preset_data))
				{
					++xcsc_replay_status;
				}
				break;
			case 1:
				if (check_and_switch_current_replay_preset(xcsc_start_preset_num_of_commands, xcsc_start_preset_cmd_type, 
						xcsc_start_preset_size, xcsc_start_preset_addr, xcsc_start_preset_data))
				{
					++xcsc_replay_status;
				}
				break;
			case 2:
				if (check_and_switch_current_replay_preset(xcsc_stop_preset_num_of_commands, xcsc_stop_preset_cmd_type, 
						xcsc_stop_preset_size, xcsc_stop_preset_addr, xcsc_stop_preset_data))
				{
					++xcsc_replay_status;
				}
				break;
...
static u32 xcsc_read(struct xcsc_dev *xcsc, u32 reg)
{
	...
	{
		check_and_switch_to_next_presets_4_xcsc();
    ...
    iowrite32(reg, iomem_addr_xcsc);
    iowrite32(32, iomem_addr_xcsc + 4);
	...
	// record_next_sec_command(iomem_addr_xcsc, SEC_REPLAY_TYPE_READ, 32, reg, temp_reading_data);
...
static void xcsc_write(struct xcsc_dev *xcsc, u32 reg, u32 data)
{
	...
	// lock_recording_mutex(1);
	// lock_irq_status_recording_mutex(1);
	// record_next_sec_command(iomem_addr_xcsc, SEC_REPLAY_TYPE_WRITE, 32, reg, data);
	// lock_irq_status_recording_mutex(0);

    u32 done_indicator = 32;

	iowrite32(reg, iomem_addr_xcsc + 8);
	iowrite32(data, iomem_addr_xcsc + 12);
	iowrite32(32, iomem_addr_xcsc + 16);
